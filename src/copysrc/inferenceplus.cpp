...
/*
void DisSeqOrderInferEngine::forward(State &state, int T, 
      RewardMap &rewards, vector<vector<vector<vector<double> > > > &partition) {
   ...
   partition.resize(T, vector<vector<vector<double> > >(dims.first, 
   ...
   double maximum;
   // TODO: modify the dijkstras method
   rewards.dijkstras(state.pos, costs, order, maximum);
   // this set order an vector contains position pairs in order of 
   // distances from the pos 
   // At present, the heuristic ignors the effect of velocity
   ...
   for (int t=0; t < (T-1); t++) {
      ...
      for (int i=0; i < order.size(); i++) { 
		 ...
		 for (int v = 0;v < v_dim;v++){
		    mask.at(x).at(y).at(v) = true;
			State s(x,y,v);
            selectProp(s, partition.at(t), partition.at(t), 
...
void DisSeqOrderInferEngine::backward(State &state, int T, 
   ...
   partition.at(0) = initial;//initBackWeight
   ...
   for (int t=0; t < (T-1); t++) {
      ...
      for (int i=order.size()-1; i >= 0; i--) { 
		 ...
		 for (int v = 0;v < v_dim;v++){
		    mask.at(x).at(y).at(v) = true;
            State s(x,y,v);
            selectBackProp(s, partition.at(t), partition.at(t), 
               partition.at(t+1), mask, rewards); 
		 }
      } 
   }
...
void DisSeqOrderInferEngine::forward(State &state, int T, 
   ...
   partition.resize(T, vector<vector<vector<double> > >(dims.first, 
	   ...
	   cerr<<"forward: "<<T<<" "<<dims.first<<" "<<dims.second<<
		   " "<<v_dim<<" "<<ba.what()<<endl;
   }
   ...
   double maximum;
   State* start = &state;
   rewards.dijkstras(start, costs, order, maximum);
   int update_size = floor(order.size()/2);
   //cout << "Forward Iteration size: "<<update_size<<endl;
...
void DisSeqOrderInferEngine::backward(State &state, int T, 
   ...
   partition.resize(T, vector<vector<vector<double> > >(dims.first, 
	   ...
	   cerr<<"backward: "<<T<<" "<<dims.first<<" "<<dims.second<<
		   " "<<v_dim<<" "<<ba.what()<<endl;
   }
   ...
   rewards.dijkstras(start, costs, order, maximum); 
   int update_size = floor(order.size()/2);
   //cout << "Backward Iteration size: "<<update_size<<endl;
...
DisSeqPredictor::DisSeqPredictor(Grid &_grid, RewardMap &_rewards, 
      DisSeqOrderInferEngine &_engine, int _mode) 
      : grid(_grid), rewards(_rewards), engine(_engine),mode(_mode) {
...
double DisSeqPredictor::predict(State &current, 
   ...
   try{
	   vector<vector<vector<double> > > threeD (
			   dimensions[0],vector<vector<double> > 
			   (dimensions[1], vector<double>(dimensions[2],-HUGE_VAL)));
       cout<<"Declare 3D"<<endl;
	   cummBackPartition.resize(10,threeD);
   }catch(bad_alloc &ba){
	   cerr<<"cummBackParttion resize: "<<dimensions[0]<<" "<<dimensions[1]<<" "
		   <<dimensions[2]<<
		   " "<<ba.what()<<endl;
   }
