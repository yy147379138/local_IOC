...
class InferenceEngine {
   public:
      ...
      InferenceEngine(Grid &_grid, FeatureArray &_feats, 
            int _connections=GRID8) 
         : grid(_grid), features(_feats), connections(_connections) { }
...
class WaveInferenceEngine : public InferenceEngine {
   public:
      WaveInferenceEngine(Grid &_grid, FeatureArray &_feats, int _connections) 
         : InferenceEngine(_grid, _feats, _connections) { }
...
class OrderedWaveInferenceEngine : public WaveInferenceEngine {
   public:
      OrderedWaveInferenceEngine(Grid &_grid, FeatureArray &_feats, 
            int _connections)
         : WaveInferenceEngine(_grid, _feats, _connections) { }
...
class DisSeqOrderInferEngine {
   public:
      ...
      DisSeqOrderInferEngine(Grid &_grid, FeatureArray &_feats, SeqFeature &_sfeat,
            int _connections=GRID8) 
         : grid(_grid), features(_feats), seqfeat(_sfeat), connections(_connections) { }

      void forward(State &state, int T, RewardMap &rewards, 
           vector<vector<vector<vector<double> > > > &partition);

      void backward(State &state, int T, RewardMap &rewards,
...
class DisSeqPredictor {
   public:
	  ...
	  ~DisSeqPredictor(){
		  ...
		  delete forwardPartitionA;
		  delete forwardFlatPartitionA;
		  delete forwardPartitionB;
		  delete forwardFlatPartitionB;
		  delete backwardPartition;
		  delete cummBackPartition;
		  delete initBackWeight;
      }
      void setStart(State & start);
      void setPrior(vector<vector<vector<double> > > &_prior);
      double predict(State &current, 
